...
|
    T_INTEGER T_ARRAY T_ID T_SOPEN expression T_SCLOSE T_SEMICOLON
    {
        ...
        for (int i=0;i< $5 -> val;i++)
        {
            ...
            std::stringstream ss;
            ss<<*$3<<num;
            symbol_table[ss.str()] = var_data( d_loc__.first_line, integer, new_label());
        }
        symbol_table["tmp"] = var_data( d_loc__.first_line, integer, new_label());
        delete $3;
        delete $5;
...
assignment: 
    ...
    {
         if($3->expr_type != integer && $6 ->expr_type != integer)
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
         ...
         std::stringstream ss;
         ss<<*$1<<$3->val;
         if(symbol_table.count(ss.str()) == 0)
         {
            std::stringstream tt;
            tt << "Undeclared variable: " << ss.str() << std::endl;
            error( tt.str().c_str() );
         }

        $$ = new std::string($6->expr_code +
                             "mov [" + symbol_table[ss.str()].label +"] ,eax\n"       );
    }
|
...
|
    T_ID T_SOPEN expression T_SCLOSE
    {
        ...
        $$ = new expression_descriptor(integer,"mov eax, [" + symbol_table[ss.str()].label + "]\n");
        delete $1;
        delete $3;
    }
|
