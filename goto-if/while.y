...
|
    T_ID T_COLON
    {
        ...
        {
            std::stringstream ss;
            ss << "Re-declared variable: " << *$1 << ".\n"
            << "Line of previous declaration: " << symbol_table[*$1].decl_row << std::endl;
            error( ss.str().c_str() );
        }
        symbol_table[*$1] = var_data( d_loc__.first_line, jumplabel, new_label() );
...
|
    T_GOTO T_ID T_IF expression T_SEMICOLON
    {
        ...
        std::string label = symbol_table[*$2].label;
        $$ = new std::string($4->expr_code +
                            "cmp al,1\n" +
                            "je near " + label +"\n"
                            );
        delete $2;
