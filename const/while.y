...
program:
    T_PROGRAM T_ID declarations T_BEGIN statements T_END
    {
        ...
        {
            if(v.second.decl_type == integer && v.second.is_const == 0)
                std::cout << v.second.label << ": resd 1" << std::endl;
            if(v.second.decl_type == boolean && v.second.is_const == 0)
                std::cout << v.second.label << ": resb 1" << std::endl;
        }
        std::cout << "section .data" << std::endl;
        for(std::pair<std::string,var_data> v : symbol_table)
        {
            if(v.second.decl_type == integer && v.second.is_const == 1)
                std::cout << v.second.label << ": dd "<< v.second.default_val << std::endl;
            if(v.second.decl_type == boolean && v.second.is_const == 1)
                std::cout << v.second.label << ": db "<< v.second.default_val << std::endl;
        }
        std::cout << std::endl;
...
|
    T_CONST T_INTEGER T_ID T_EQ expression T_SEMICOLON
    {
        ...
        symbol_table[*$3] = var_data( d_loc__.first_line, integer, new_label() ,$5 -> value ,1);
        delete $3;
        delete $5;
    }
|
    T_CONST T_BOOLEAN T_ID T_EQ expression T_SEMICOLON
    {
        if( symbol_table.count(*$3) > 0 )
        {
            std::stringstream ss;
...
assignment:
    ...
    {
        if(symbol_table[*$1].is_const == 1)
        {
            std::stringstream ss;
            ss << "Const variable can not be used here: " << *$1 << std::endl;
            error( ss.str().c_str() );
        }
